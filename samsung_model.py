...
class LSTM_RNN(object):
	...
	def __build__(self):
		...
		self.optimizer =tf.train.AdamOptimizer(self.learning_rate,
		 beta1=0.9, beta2=0.999).minimize(self.cost)
		...
		# accuracy = sess.run(self.logits, feed_dict={
		#     self.prediction: get_accuracy, self.target:y_eval
		#     })
	...
	def create_training_data(self):
		...
		data0, data1, data2, data3, data4 = data[:,1], data[:, 2], data[:, 3], data[:, 5], data[:, 4]
		data0, data1, data2, data3, data4 = data0.reshape(-1,1), data1.reshape(-1,1), data2.reshape(-1,1), data3.reshape(-1,1), data4.reshape(-1,1)
		
		data = np.concatenate((data0, data1, data2, data3, data4), axis =1)
		data = data[0:3000, :]

		print("test_data", data[0:7])
		print("test_pred", data[7])

		print(data.shape)
		print("data0", data[0:1])
